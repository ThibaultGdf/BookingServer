...
const destroy = async function(req, res) {
    try {
        ...
        if(!user) {
            res.status(404).json({ message: `L'utilisateur ${id} n'existe pas !`})
        }
        ...
        res.status(200).json({ message: `L'utilisateur ${id} a bien été supprimé !` });

    } catch(error) {
        res.status(500).json({ message: `Erreur serveur lors de la suppression d'un utilisateur !` });
    }
}
